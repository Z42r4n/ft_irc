...
/*   Updated: 2023/11/09 12:58:41 by ymoutaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <ircserv.hpp>

void Server::bimoCommand(t_fd fd, t_params params)
{	
	std::string api = "";
	...
	else
	{
		...
		sendData(fd, RPL_MOTD(clients[fd].getNickname(), "Usage: /BIMO [fonts])"));
		sendData(fd, RPL_MOTD(clients[fd].getNickname(), "Usage: /BIMO [figlet] [font] [text])"));
		sendData(fd, RPL_MOTD(clients[fd].getNickname(), "Usage: /BIMO [weather])"));
		return ;
	}
	...
	sendData(fd, RPL_MOTD(clients[fd].getNickname(), "BIMO BOT RESPONSE ------------------"));
	...
	sendData(fd, RPL_MOTD(clients[fd].getNickname(), "BIMO RESPONSE END ------------------"));
